...
#' The column "ErrorTarget" == FALSE when the condition on BIN/INT/ORD/CAT on the target is respected
#' @export
#'
check_naming_conventions <- function(data, test_target_type=FALSE) {
  ...
  ## naming conventions
  ## all variables except target
  ## Type_Source_Concept_Aggregation_AggregationPeriod_PeriodTime
  ## Ou Type_Source_Concept_Aggregation_AggregationPeriod_Derivations
  ## target (type =Z)
  ## Type_Source_Concept_BIN/INT/ORD/CAT_TypeOfPrediction
  ...
  for (i in seq_along(varnames)) {
    ...
    if (
        ( (type %in% c("N_", "M_", "F_", "R_", "P_", "Y_","J_")) &(cl %in% c("numeric", "integer")) )
      |
        ( (type %in% c("D_","S_")) & (cl %in% c("Date", "POSIXct", "POSIXlt")))
      |
        ( (type %in% c("C_", "T_", "L_")) & (cl %in% c("factor", "character", "logical", "integer","numeric")) )
      |
        ( (type %in% c("E_", "I_", "A_", "O_", "X_", "Z_", "K_", "W_", "Q_")) )
      )  {
        out[ivar, "ErrorType"] <- FALSE
    } else out[ivar, "ErrorType"] <- TRUE

    # test on the target type
    if (test_target_type & type =='Z'){
      target_type <- splitw[nb_words] # last element
