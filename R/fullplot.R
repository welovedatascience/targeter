#' @title fullplot
#' @description autoplot method for crossvar OR targrter object (with a variable specification).
#' draws the combination of 4 selected plots/

#' @param x object of class 'crossvar' or 'targeter'
#' @param var character, used when x is a targeter object to select the variable of interest (crossvar in x$profiles slot)
...
#' @param subtitle boolean: do we plot default generated subtitle Y/N or character (override subtitle)- default: TRUE
#'
#'
...
fullplot <- function(x,
  ...
  if (x$target_type %in% c('binary','categorical')){
      ...
      g1 <- plot.crossvar(x, show='boxplot',
                          metadata=metadata,
    ...
    if (x$variable_type %in% c('character')) {
      ...
      if (is.logical(subtitle)){
        if (subtitle) {
          subtitle <- 'boxplot, frequencies (N), WOE'
          if (L_quad) subtitle <- paste(subtitle, ', quadrant (N/avg)')
        }
        else subtitle <- NULL
