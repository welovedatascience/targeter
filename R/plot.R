...
plot.crossvar_numeric <- function(x,
  ...
  plotValues <- function(dfm, forNA=FALSE){
    ...
    p1 <- p1+ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, hjust = 1))
...
plot.crossvar_categorical <- function(x,
  ...
  ##this command allows to make shortcut during the function call and to be sure to enter the good values
  show <- match.arg(show, c("counts","props","index","woe"), several.ok = FALSE)
  type <- match.arg(type, c("auto","bars","line"), several.ok = FALSE)
  ...
  if (type == "auto"){
    type <- switch (show,"counts" = "bars","props" = "line","index" = "line", "woe" = "bars")
  ...
  plotValues <- function(dfm, forNA=FALSE){
    ...
    if (type=="bars"){
      ...
      if(x$target_type == "binary"){
        if(!forNA){
          p1 = p1 + ggplot2::scale_fill_manual(
            breaks=vbreaks,
            labels=vlabels,
            values = vcolors)
        } else{
    ...
    if(any(c(TRUE,FALSE) %in% unique(dfm[,1]))){
      p1 = p1 + ggplot2::scale_x_discrete(breaks=c("FALSE","TRUE","[Missing]"),labels=c("FALSE", "TRUE","MISSING"))
...
quadrant_plot <- function(x,metadata=NULL, max_ncat=15, print_NA=TRUE){
  ...
  assertthat::assert_that(inherits(x,"crossvar"), msg = "the parameter x must to be an object of class crossvar")
  assertthat::assert_that(inherits(metadata,"data.frame")| is.null(metadata), msg = "The parameter metadata must be either NULL (no metadata) or a data.frame")
  assertthat::assert_that(requireNamespace('ggrepel', quietly = TRUE), msg='ggrepel package required')
  assertthat::assert_that(requireNamespace('scales', quietly = TRUE), msg='scales package required')



  assertthat::assert_that(x$target_type %in% c('binary','numeric'),
  ...
  all$color <- ifelse(all$names=='[Missing]','darkred',
                      ifelse(all$names=='[Other]',
                             'darkblue',
                             'black'))
  ...
  ylabel <- ifelse(x$target_type=='binary', "Target penetration  (%)", "Target mean")
  ...
  p1 <- p1+  ggplot2::scale_x_continuous(label = scales::label_comma(accuracy = 1))
