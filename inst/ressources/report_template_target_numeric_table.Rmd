...
targeter.table <- function (x, ...) 
{
    if (x$target_type %in% c("binary", "categorical")) {
        out <- (cbind(as.data.frame.matrix(x$counts), as.data.frame.matrix(x$props)))
    }
...
tari <- targeter.table(iprofile)
cat(kableExtra::kable_styling(kableExtra::kable(tari, kable_format ='html'),position = "left", full_width = FALSE))


# tab <- DT::datatable(tari, rownames = TRUE,  
#               selection="none", autoHideNavigation = TRUE, 
#               
